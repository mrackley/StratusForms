  ...
  $.fn.StratusFormsInitialize = function (options) {
      ...
      if (opt.htmlForm != null)
      {
          ...
          return this.each(function () {
              ...
              gStratusFormsWebURL = opt.webURL;
      ...
      return this.each(function () {
          ...
          if (opt.validateForm) {
              if (($this).StratusFormsValidate({
                  ...
                  saveForm($this, opt.listName, opt.completefunc, opt.StratusFormsDataField,opt.formID);
              }
              else {
...
//                value = value.replace(regex, "<br>");
              ...
              $.extend( gStratusFormsFormData, formData );
              $.extend( gStratusFormsFormData, listFields );

              $("#CreatedBy").html(createdBy);
              $("#CreatedDate").html(created);

              PopulateFormData(form, gStratusFormsFormData, listFields);
  ...
  function PopulateFormData(form, formData,listFields,repeatableRowNum) {
      ...
      for (var field in formData) {
          ...
          if (field === "StratusFormsRepeatable") {
            ...
            var useValue = false;
            if ($(element).hasClass("SFStoreValue"))
                useValue = true;
          ...
          if ($(element).attr("data-StratusFormsLookup") != undefined )
          {
            ...
            var peopleValue = value;
              var valueArray = value.split(";#");
              if (valueArray.length > 1)
              {
                  for(x = 1; x < valueArray.length; x+=2)
                  {
                      if(x == 1)
                          value = valueArray[x];
                      else
              ...
              if ($(element).attr("data-StratusFormsType") != undefined && $(element).attr("data-StratusFormsType") == "PeoplePicker") {
                $(element).StratusFormsPeoplePicker();
                  ...
                  if (peopleValue.length > 0) {
                      var people = htmlDecode(peopleValue).split(";#");
      ...
      var value = listFields[$(element).attr("ListFieldName")];
            if (value == undefined)
            {
              ...
              if($(element).is(':checkbox') && value == 1)
              {
  ...
  function CDataWrap(value,validate) {
    //  if (validate != undefined && validate == "validNumber")
      //    return value;
      return "<![CDATA[" + value + "]]>";
  }
  ...
  function buildStratusFormsDataObject(formElement,StratusFormsValuePairs) {
          ...
          if (id.indexOf("TopSpan_HiddenInput") < 0) {
              ...
              if (value != undefined && type.toUpperCase() != "BUTTON") {
                  ...
                  if (formVal != undefined) {
                      if ($(this).attr("isDate") == "yes" && value != undefined && value.length > 0) {
                          var thisDate = new Date(value);
                          value = thisDate.toISOString();
                      }
                      if ((encryptField && !gStratusFormsDecryptFailed) || !encryptField) {
                          StratusFormsValuePairs.push([$(this).attr("listFieldName"), CDataWrap(value,$(this).attr("validate"))]);
                      }
                  }
      ...
      if ($(this).attr("data-StratusFormsOptions") != undefined )
    {
       eval("options=" + $(this).attr("data-StratusFormsOptions"));
          ...
          value = ""; //$(this).find("option:selected").text();
           $(this).find(":selected").each(function(i, selected){
              if(value != "")value += ";#";
              if($(select).hasClass("SFStoreValue"))
              {
                value += $(selected).val();
              } else {
                value += $(selected).text();
      ...
      $(formElement).find("div[data-StratusFormsType='PeoplePicker']").each(function () {
          var divID = $(this).attr("id") + "_TopSpan";
          if($("#" + divID).length != 0)
          {
            var people = $().StratusFormsGetPeopleFromPeoplePicker(this);
            ...
            formDataObject[this.id] = emails;
              ...
              formDataObject[this.id] = "";
          }
      });

      $(formElement).find("span[listFieldName],span[listFieldName],label[listFieldName]").each(function () {
              var value = "";
  ...
  function GetSigatureAsString(canvasID)
{
  ...
  function generateFormObject(formElement)
  {
  ...
  if (formID == null)
  {
    formID = gStratusFormsFormID;
      ...
      $().StratusFormsSaveForm(listName, formID, StratusFormsValuePairs, saveCompleteFunc,gStratusFormsChildListData, gStratusFormsFiles,gStratusFormsWebURL);
  }
...
function validCurrency(value, element, offsetLeft, offsetTop) {
  ...
  if (!(/^-?(?:\d+|\d{1,3}(?:,\d{3})+)?(?:\.\d+)?$/.test(value))) {
        ...
        $(element).val("$" + (value*1).toFixed(2).replace(/,/g,""));
    }
  }
